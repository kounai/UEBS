...
$org = mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT * FROM organizers WHERE organizerID = $organizerID"));


$organizerName = $org['name'];
$organizerRole = $org['role'];
$_SESSION['organizer_name'] = $organizerName;

$totalEvents  = (int)mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(*) AS c FROM events WHERE organizerID=$organizerID"))['c'];
$totalReg     = (int)mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(*) AS c FROM registrations r JOIN events e ON r.eventID=e.eventID WHERE e.organizerID=$organizerID"))['c'];
$totalTickets = (int)mysqli_fetch_assoc(mysqli_query($conn,
    "SELECT COUNT(*) AS c FROM requests WHERE organizerID=$organizerID"))['c'];

$current_page = 'organizer_profile.php';
  ...
  <div class="row mb-4">
    ...
    <div class="col-md-4">
      <div class="card text-center">
        ...
        <p style="color:#888;margin:0;">Total Registrations</p>
      </div>
    </div>
    <div class="col-md-4">
      <div class="card text-center">
        <h2 style="color:#e67e22;"><?= $totalTickets ?></h2>
        <p style="color:#888;margin:0;">Support Tickets</p>
      </div>
    </div>
  </div>
      ...
      <div class="card">
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control" value="<?= htmlspecialchars($org['name']) ?>" disabled style="background:#f0f2f5;">
          </div>
          <div class="col-md-6 mb-3">
            <label class="form-label" style="font-size:13px;font-weight:600;">Email Address</label>
            <input type="text" class="form-control" value="<?= htmlspecialchars($org['email']) ?>" disabled style="background:#f0f2f5;">
          </div>
          <div class="col-md-6 mb-3">
            <label class="form-label" style="font-size:13px;font-weight:600;">Role</label>
            <input type="text" class="form-control" value="<?= htmlspecialchars($org['role']) ?>" disabled style="background:#f0f2f5;">
          </div>
          <div class="col-md-6 mb-3">
            <label class="form-label" style="font-size:13px;font-weight:600;">Organizer ID</label>
            <input type="text" class="form-control" value="<?= $organizerID ?>" disabled style="background:#f0f2f5;">
          </div>
        </div>
